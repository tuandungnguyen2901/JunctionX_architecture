...
ECR_APPLICATION_REPO_NAME=backend_tier
...
ECR_PRESENTATION_REPO_NAME=frontend_tier
...
docker build -t backend_tier .
docker tag backend_tier:latest $ECR_APPLICATION_TIER_REPO:latest
...
docker build -t frontend_tier .
docker tag frontend_tier:latest $ECR_PRESENTATION_TIER_REPO:latest
